...
export function ProfessionalForm({ onSuccess, onCancel }: ProfessionalFormProps): React.JSX.Element {
  const {
  ...
  return (
    <form onSubmit={onSubmit} className="flex min-h-0 flex-1 flex-col">
      <div className="flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto px-4 pb-4">
        <div className="flex items-center gap-3 rounded-lg border bg-muted/40 p-3">
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <span className="truncate text-sm font-medium text-foreground">
              {previewName || "Nome do Profissional"}
            </span>
        ...
        <FieldGroup>
          <Field>
            <FieldLabel htmlFor="name">Nome do Profissional</FieldLabel>
            <InputForm
              name="name"
              control={control}
      ...
      <SheetFooter className="flex-row items-center justify-end gap-3 border-t px-4 py-4">
        ...
        <Button type="submit" disabled={isPendingCreateBarber}>
          {isPendingCreateBarber ? "Cadastrando..." : "Cadastrar Profissional"}
        </Button>
      </SheetFooter>
    </form>
  )
